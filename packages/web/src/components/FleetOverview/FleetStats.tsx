...
export function FleetStats({ agents, locks }: Props) {
  ...
  const stats = [
    ...
    { label: 'Completed', value: completed, icon: CheckCircle, color: 'text-th-text-muted', bg: 'bg-gray-500/10' },
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
      {stats.map((s) => (
        <div
          key={s.label}
          className={`${s.bg} border border-th-border/50 rounded-lg p-3 flex items-center gap-3`}
        >
          <s.icon size={18} className={s.color} />
          <div>
            <div className={`text-lg font-bold ${s.color}`}>{s.value}</div>
            <div className="text-[10px] text-th-text-muted uppercase tracking-wider">{s.label}</div>
          </div>
        </div>
      ))}
    </div>
