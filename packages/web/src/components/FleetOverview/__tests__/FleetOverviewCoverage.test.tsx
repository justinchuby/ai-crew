...
describe('FleetOverview — coverage', () => {
  ...
  it('renders basic structure with no agents', () => {
  ...
  it('renders agent filter dropdown when agents exist', () => {
    mockAppState.agents = [
  ...
  it('shows heatmap section when >= 2 agents', () => {
    mockAppState.agents = [
      { id: 'a1', role: { id: 'dev', name: 'Developer', icon: '💻' }, status: 'running' },
  ...
  it('toggles heatmap visibility', () => {
    ...
    fireEvent.click(screen.getByText('Communication Heatmap'));
    expect(screen.getByTestId('comm-heatmap')).toBeInTheDocument();
    fireEvent.click(screen.getByText('Communication Heatmap'));
    expect(screen.queryByTestId('comm-heatmap')).not.toBeInTheDocument();
  });

  it('does not show heatmap section with < 2 agents', () => {
    mockAppState.agents = [
      { id: 'a1', role: { id: 'dev', name: 'Developer', icon: '💻' }, status: 'running' },
    ];
    render(<FleetOverview />);
