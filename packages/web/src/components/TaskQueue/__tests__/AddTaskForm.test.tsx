...
describe('AddTaskForm', () => {
  it('renders title, role, and description fields', () => {
    render(<AddTaskForm projectId="p1" onCreated={vi.fn()} onClose={vi.fn()} />);
  ...
  it('disables submit when title or role empty', () => {
    render(<AddTaskForm projectId="p1" onCreated={vi.fn()} onClose={vi.fn()} />);
    const submitBtn = screen.getByText('Add Task') as HTMLButtonElement;
    expect(submitBtn.disabled).toBe(true);
  });

  it('enables submit when title and role filled', () => {
    render(<AddTaskForm projectId="p1" onCreated={vi.fn()} onClose={vi.fn()} />);
  ...
  it('calls apiFetch on submit and triggers callbacks', async () => {
    ...
    const onClose = vi.fn();
    render(<AddTaskForm projectId="p1" onCreated={onCreated} onClose={onClose} />);
    fireEvent.change(screen.getByLabelText('Task title'), { target: { value: 'Fix bug' } });
    fireEvent.change(screen.getByLabelText('Task role'), { target: { value: 'developer' } });
  ...
  it('calls onClose when Cancel clicked', () => {
    const onClose = vi.fn();
    render(<AddTaskForm projectId="p1" onCreated={vi.fn()} onClose={onClose} />);
