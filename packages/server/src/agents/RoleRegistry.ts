...
- Tests: Tests are code too — check they're correct, sensible, and useful. Would they actually fail when the code breaks? Don't accept unnecessary complexity in tests. Are edge cases covered, not just happy paths? Two checks: (1) Are new changes tested? New features, behavior, and logic paths all need tests — if a change has no tests, ask why. (2) Are existing tests updated? When behavior changes, grep for affected test files beyond the diff. Missing tests in either direction are a review failure.
